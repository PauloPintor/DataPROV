...
select sum(lineitem.l_extendedprice) / 7.0 as avg_yearly from lineitem, part where part.p_partkey = lineitem.l_partkey and part.p_brand = 'Brand#34' and part.p_container = 'LG CASE' and lineitem.l_quantity < ( select 0.2 * avg(lineitem.l_quantity) from lineitem.lineitem where lineitem.l_partkey = part.p_partkey );
...
-- Q19
select sum(lineitem.l_extendedprice* (1 - lineitem.l_discount)) as revenue from lineitem, part where ( part.p_partkey = lineitem.l_partkey and part.p_brand = 'Brand#24' and part.p_container in ('SM CASE', 'SM BOX', 'SM PACK', 'SM PKG') and lineitem.l_quantity >= 3 and lineitem.l_quantity <= 3 + 10 and part.p_size between 1 and 5 and lineitem.l_shipmode in ('AIR', 'AIR REG') and lineitem.l_shipinstruct = 'DELIVER IN PERSON' ) or ( part.p_partkey = lineitem.l_partkey and part.p_brand = 'Brand#35' and part.p_container in ('MED BAG', 'MED BOX', 'MED PKG', 'MED PACK') and lineitem.l_quantity >= 15 and lineitem.l_quantity <= 15 + 10 and part.p_size between 1 and 10 and lineitem.l_shipmode in ('AIR', 'AIR REG') and lineitem.l_shipinstruct = 'DELIVER IN PERSON' ) or ( part.p_partkey = lineitem.l_partkey and part.p_brand = 'Brand#22' and part.p_container in ('LG CASE', 'LG BOX', 'LG PACK', 'LG PKG') and lineitem.l_quantity >= 25 and lineitem.l_quantity <= 25 + 10 and part.p_size between 1 and 15 and lineitem.l_shipmode in ('AIR', 'AIR REG') and lineitem.l_shipinstruct = 'DELIVER IN PERSON' );
...
select cntrycode, count(*) as numcust, sum(customer.c_acctbal) as totacctbal from ( select substring(customer.c_phone from 1 for 2) as cntrycode, customer.c_acctbal from customer where substring(customer.c_phone from 1 for 2) in ('33', '19', '17', '11', '23', '24', '27') and customer.c_acctbal > ( select avg(customer.c_acctbal) from customer where customer.c_acctbal > 0.00 and substring(customer.c_phone from 1 for 2) in ('33', '19', '17', '11', '23', '24', '27') ) and not exists ( select * from orders where orders.o_custkey = customer.c_custkey ) ) as custsale group by cntrycode order by cntrycode;
