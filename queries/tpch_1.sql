...
select p_brand, p_type, p_size, count(distinct ps_suppkey) as supplier_cnt from partsupp, part where p_partkey = ps_partkey and p_brand <> 'Brand#42' and p_type not like 'ECONOMY ANODIZED%' and p_size in (13, 36, 1, 39, 18, 8, 10, 25) and ps_suppkey not in ( select s_suppkey from supplier where s_comment like '%Customer%Complaints%' ) group by p_brand, p_type, p_size order by supplier_cnt desc, p_brand, p_type, p_size;

-- Q17
select sum(lineitem.l_extendedprice) / 7.0 as avg_yearly from lineitem, part where p_partkey = lineitem.l_partkey and part.p_brand = 'Brand#24' and part.p_container = 'LG BOX' and lineitem.l_quantity < ( select 0.2 * avg(lineitem.l_quantity) from lineitem.lineitem where lineitem.l_partkey = part.p_partkey );
...
-- Q21 not supported by the current version of the system
select cntrycode, count(*) as numcust, sum(c_acctbal) as totacctbal from ( select substring(c_phone from 1 for 2) as cntrycode, c_acctbal from customer where substring(c_phone from 1 for 2) in ('28', '18', '13', '14', '22', '15', '10') and c_acctbal > ( select avg(c_acctbal) from customer where c_acctbal > 0.00 and substring(c_phone from 1 for 2) in ('28', '18', '13', '14', '22', '15', '10') ) and not exists ( select * from orders where o_custkey = c_custkey ) ) as custsale group by cntrycode order by cntrycode;
