...
-- Q16
select part.p_brand, part.p_type, part.p_size, count(distinct partsupp.psupplier.s_suppkey) as supplier_cnt from partsupp, part where part.p_partkey = partsupp.psupplier.s_partkey and part.p_brand <> 'Brand#51' and part.p_type not like 'ECONOMY PLATED%' and part.p_size in (44, 49, 13, 20, 36, 19, 35, 25) and partsupp.psupplier.s_suppkey not in ( select supplier.s_suppkey from supplier where supplier.s_comment like '%Customer%Complaints%' ) group by part.p_brand, part.p_type, part.p_size order by supplier_cnt desc, part.p_brand, part.p_type, part.p_size;

-- Q17 not supported by the current version of the system
select sum(lineitem.l_extendedprice) / 7.0 as avg_yearly from lineitem, part where part.p_partkey = lineitem.l_partkey and part.p_brand = 'Brand#14' and part.p_container = 'WRAP BAG' and lineitem.l_quantity < ( select 0.2 * avg(lineitem.l_quantity) from lineitem.lineitem where lineitem.l_partkey = part.p_partkey );
...
select cntrycode, count(*) as numcust, sum(customer.c_acctbal) as totacctbal from ( select substring(customer.c_phone from 1 for 2) as cntrycode, customer.c_acctbal from customer where substring(customer.c_phone from 1 for 2) in ('18', '27', '19', '11', '22', '33', '15') and customer.c_acctbal > ( select avg(customer.c_acctbal) from customer where customer.c_acctbal > 0.00 and substring(customer.c_phone from 1 for 2) in ('18', '27', '19', '11', '22', '33', '15') ) and not exists ( select * from orders where orders.o_custkey = customer.c_custkey ) ) as custsale group by cntrycode order by cntrycode;
